---
title: "Bioinformatics tools to analyse complex diseases"
...
We focused on developing methodologies that support the discovery process and as the results VisuNet and funMotifs were developed.

The main aim of my projects was to extend the methods and apply them to large-scale experiments created for complex diseases, e.g. autism and cancer.
...
Interactions are discovered from transparent classifiers, such as rule-based classifiers (RBCs). 
...
## A  potential important role of MAP7 and COX2 interdependency  in autism {data-background-image="images/UU-figures-en-2019copy.jpg"}
...
- VisuNet is conceptually simple, but useful for discovery the interdependencies  in RBCs,
- RN identifies different combinations of features and their discretisation levels that are associated with decisions,
- RN helps explain the differences between the classes,
- The autism-control RN confirms some known genes associated with autism,
- RN may suggest a new functionality for other attributes, e.g. MAP7 and CAPS2
...
To understand how the genes are regulate, we need to know what determined functionality of TF motifs. 
...
- SNPs from GWAS Catalogue and SNPs in LD with GWAS SNPs (r>0.8)(KS)
...
- Functional annotations help to identify regulatory mutations that may significantly deregulate gene expression in cancer and other complex diseases,
- funMotifs applications identified potentially functional variants in cancer and normal genomes,
- Experimenatal validation may be perfomerd on the CFRMs to confirm our findings
...
- Project 2. - modification of the funMotifs algorithm to improve identification of the false positive regulatory motifs. We are planning to continue research on the functional annotation of motifs using funMotifs. We would like to extend the functional motifs definition in terms of the conservation of the individual nucleotides in TF motifs across various diseases.   
...
- Project 4. - identification of potential biomarkers that discern glioma subtypes using CFRMs from the COSMIC-available mutations for gliomas. We intersected the results with the differentially expressed genes based on the TCGA and GTEx databases.
